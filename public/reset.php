...
<?php $form->checkGet('passwordCheck') ? 
		$database->checkValue('users','passwordCheck',$form->getGet('passwordCheck')) ?
			$passwordCheck = $form->getGet('passwordCheck')
			: $page->redirect('home.php')
		: $page->redirect('home.php');
?>
<?php $session->checkValue('loginId') or $cookie->checkValue('loginId') ? $page->redirect('home.php') : null; ?>
<?php
...
if($form->checkPost('email') and 
	...
	if($session->getValue('error') == ''){
		...
		$username = $database->otherValue('users','email',$email,'username');
